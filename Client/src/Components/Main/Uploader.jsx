...
import { Icon_Uploading, Icon_UploadingError, Icon_UploadingSuccess } from '../../Assets/img/Main/index.js'
        ...
        <span className={`Uploader${Object.keys(Service.uploading).length ? ' show' : ''}`}>
            {
                Object.keys(Service.uploading)
                    ...
                    .map(el => <UploadingBlock key={el.name} {...el} />)
            }
        </span>   
    )
}

let UploadingBlock = ({ status, name, size, progress, hide }) => {
    let Icon;
