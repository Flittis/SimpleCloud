...
import { Icon_Folder, Icon_Document, Icon_Video, Icon_Image, Icon_Audio, Icon_Archive, Icon_Book, Icon_Delete_Red, Icon_Access, Icon_Share } from '../../Assets/img/Main'

let FileBlock = ({ File }) => {
    let { Service } = useContext(Context)

    let { _id, name, type, mimetype, user, size } = File
        ...
        return (
            <block className='File__context'>
                <row className='context__row'>
                    <img className='context__row-icon' src={Icon_Share} alt='delete'/>
                    <p className='context__row-title'>Copy link</p>
                </row>
                <row className='context__row'>
                    <img className='context__row-icon' src={Icon_Access} alt='delete'/>
                    <p className='context__row-title'>Access</p>
                </row>
                <row className='context__row delete' onClick={e => { setContextShow(false); Service.delete({ _id }) }}>
            ...
            <Link to={type === 'folder' ? `/o/${_id}` : `/f/${_id}`} onContextMenu={e => { e.preventDefault(); setContextShow(!contextShow) } }>
                <block className='content__block'>
                    ...
                    <div className='content__block-meta'>
                        ...
                        <p className='meta__size'>{formatBytes(size)}</p>
                    </div>
                </block>
            </Link>
        </Tippy>
